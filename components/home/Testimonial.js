        ...
        <Container fluid={false}>
          ...
          <Slider {...slick_settings}>
            {Review.map((ele, index) => {
              return (
                <div key={index}>
                  <ReviewData key={index} reviewer={ele} />
                </div>
              )
            })}
          </Slider>
      ...
      <ReviewerWarper>
        <Col xs={12} md={4}>
          <Row>
            <Col>
              <ReviewerPhoto
                src={"/static/images/testimonial/" + this.props.reviewer.photo}
              />
            </Col>
          </Row>
          <Row>
            <Col>
              <ReviewerName>
                {this.props.reviewer.fullname}{" "}
                {this.props.reviewer.nickname != "" &&
                  " (" + this.props.reviewer.nickname + ")"}
              </ReviewerName>
            </Col>
          </Row>
          <Row>
            <Col>
              <ReviewerCamp>{"ค่าย " + this.props.reviewer.camp}</ReviewerCamp>
            </Col>
          </Row>
        </Col>
